...
OUTPUT_DL12 = '/home/mingot/dl3/dl3_train_v08.csv'
...
OUTPUT_MODEL =  wp + 'models/jm_patches_dl3_v08.hdf5'  # OUTPUT_MODEL = wp + 'personal/jm_patches_train_v06_local.hdf5'
LOGS_PATH = wp + 'logs/%s' % 'malign_v08' #str(int(time()))
...
common.multiproc_crop_generator(filenames_train,
                                os.path.join(PATCHES_PATH,'dl3_v08_x_train.npz'),
                                os.path.join(PATCHES_PATH,'dl3_v08_y_train.npz'),
                                __load_and_store,
                                parallel=True)


common.multiproc_crop_generator(filenames_test,
                                os.path.join(PATCHES_PATH,'dl3_v08_x_test.npz'),
                                os.path.join(PATCHES_PATH,'dl3_v08_y_test.npz'),
                                __load_and_store,
                                parallel=True)
...
#         y_new = y.copy()
#         if is_training:
#             # canvia alguns 1's per 0's
#             ones = [s for s in selected_samples if y[s,0]==1]
#             y_new[random.sample(ones, int(0.1*len(ones))), 0] = 0
...
#     """
#     while 1:
#         selected_samples = range(len(y)) ##random.sample(range(len(y)), 1000)
#
#         #selected_samples  = [i for i in range(len(y_orig)) if y_orig[i]==1 or random.randint(0,9)==0]
...
# x_train = np.load(os.path.join(PATCHES_PATH, 'dl3_v07_x_train.npz'))['arr_0']
# y_train = np.load(os.path.join(PATCHES_PATH, 'dl3_v07_y_train.npz'))['arr_0']
# y_train = np.expand_dims(y_train, axis=1)
# x_test = np.load(os.path.join(PATCHES_PATH, 'dl3_v07_x_test.npz'))['arr_0']
# y_test = np.load(os.path.join(PATCHES_PATH, 'dl3_v07_y_test.npz'))['arr_0']
...
#
# # Load model
# model = ResnetBuilder().build_resnet_50((3,60,60),1)
