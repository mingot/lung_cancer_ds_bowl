...
OUTPUT_DL1 =  wp + 'output/noduls_patches_hardnegatives_v01.csv'  # OUTPUT_DL1 = wp + 'personal/noduls_patches_v06.csv'
...
def chunk_generator(filenames, nodules_df, thickness=0, batch_size=32, is_training=True):
    while 1:
        ...
        for X_batch, y_batch in data_generator.flow(X, y, batch_size=batch_size, shuffle=is_training):
            ...
            if i*len(X_batch) > len(X)*2:  # stop when we have augmented enough the batch
