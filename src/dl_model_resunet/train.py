...
import random
import sys
sys.path.append('/mnt/c/Users/Miguel/Downloads/lung_cancer_ds_bowl/')
...
TEST_SIZE = 5 # in number of patients
NUM_EPOCHS = 10
...
# DATASET CREATION --------------------------------
annotations_df = pd.read_csv("lung_cancer_ds_bowl/data/luna/annotations.csv")
names = [s.split('.')[-1] for s in annotations_df.seriesuid]
names = list(set(names)) # we only keep unique names
names = ['luna_'+name+'.npz' for name in names]

#file_list = [g for g in os.listdir(input_path) if g.startswith('luna_')]
file_list = names # because we will only train with the patients that at least have one nodule!!!!!!!!!!!!!!!!
...
model = ThickRESUNET(input_shape=(3,512,512), lr=1.0e-5) # load a file to continue a training (optional)

# PARAMETERS -----------------------------------
input_path = '/mnt/hd2/preprocessed5'
logs_path = '/home/mteixido/logs'
modeloutput_file = '/home/mteixido/thickresmodel5.hdf'
TEST_SIZE = 1 # in number of patients
NUM_EPOCHS = 50
SAMPLES_PER_EPOCH = 400 # number of thick slices to train per epoch
...
model.train(nb_epochs=NUM_EPOCHS,  #number of epochs to train
              ...
              max_slices=6)   # to increase suffling